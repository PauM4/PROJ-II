...
int PathFinding::CreatePath(const iPoint& origin, const iPoint& destination)
{
	...
	// L12: DONE 1: if origin or destination are not walkable, return -1
	if (IsWalkable(origin) && IsWalkable(destination))
	{
		// L12: DONE 2: Create two lists: open, close
		PathList open;
		PathList closed;
		...
		{
			// L12: DONE 3: Move the lowest score cell from open list to the closed list
			ListItem<PathNode>* lowest = open.GetNodeLowestScore();
			ListItem<PathNode>* node = closed.list.Add(lowest->data);
			open.list.Del(lowest);

			// L12: DONE 4: If we just added the destination, we are done!
			...
			// L12: DONE 5: Fill a list of all adjancent nodes
			PathList adjacent;
			node->data.FindWalkableAdjacents(adjacent);

			// L12: DONE 6: Iterate adjancent nodes:
